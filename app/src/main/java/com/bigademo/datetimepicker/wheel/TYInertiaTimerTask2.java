...
final class TYInertiaTimerTask2 extends TimerTask
{
    float a;
    final float velocityY;
    final TYLoopView loopView;

    TYInertiaTimerTask2(TYLoopView loopview, float velocityY) {
        super();
        loopView = loopview;
    ...
    public final void run() {
        ...
        if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
            ...
            loopView.handler.sendEmptyMessage(TYMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        int i = (int) ((a * 10F) / 1000F);
        TYLoopView loopview = loopView;
